...
int collisionCheck(unsigned char *collisionMap, int mapWidth, int col, int row, int width, int height, int colShift, int rowShift) {
    ...
    // Top Right
    int current2 = collisionMap[OFFSET(col + colShift + width - 1, row + rowShift, mapWidth)];
    if (current2 < smallestSoFar) {
        smallestSoFar = current2;
    }

    // Bottom Left
    int current3 = collisionMap[OFFSET(col + colShift, row + rowShift + height - 1, mapWidth)];
    if (current3 < smallestSoFar) {
        smallestSoFar = current3;
    }

    // Bottom Right
    int current4 = collisionMap[OFFSET(col + colShift + width - 1, row + rowShift + height - 1, mapWidth)];
    if (current4 < smallestSoFar) {
        smallestSoFar = current4;
    }
